...
class ChromosomeRepresentativeSelection:
    ...
    def get_representative_of_representatives(self):
    ...
    def plot_distance_variations(self, chromosome_name, plot_random_outliers=True, plot_approximate=True, x_range=None):
        figure_path = os.path.join('Figures', 'Representative', self.specie, 'Different_centroids', 'N')
    ...
    def find_centroid(distance_matrix, median=False, exclude_indices=None):
        ...
        # Initialize sums with infinities to handle fully NaN rows
        sums = np.full(masked_matrix.shape[0], np.inf)
        # Identify rows that are not fully NaN
        valid_rows = ~np.isnan(masked_matrix).all(axis=1)
        # Compute means for valid rows only
        sums[valid_rows] = np.nanmean(masked_matrix[valid_rows], axis=1)
...
if __name__ == '__main__':
    representative = ChromosomeRepresentativeSelection('Maize', 6, 'DSSIM', segment_length=500_000)
